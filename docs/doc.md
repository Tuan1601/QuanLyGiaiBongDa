...
-   **✅ Đăng ký tài khoản với validation mạnh (email, password phức
    tạp)**

-   **✅ Đăng nhập với JWT (Access Token + Refresh Token)**

-   **✅ Bảo mật: Login attempts (khóa tài khoản sau 5 lần sai - 15
    phút)**
...
-   **✅ Tạo giải đấu với 2 thể thức:**
    ...
    -   **Round-Robin: Vòng tròn 1 lượt (mỗi đội gặp nhau 1 lần)**

    -   **Group-Stage: Chia bảng (3-6 đội/bảng)**

-   **✅ 2 chế độ:**

    -   **Public: Ai cũng xem được**

    -   **Private: Chỉ owner hoặc người có access token**
...
-   **✅ Validation ngày tháng (không được ở quá khứ, endDate \>
    startDate)**
...
-   **✅ Unique constraint: Tên đội và tên viết tắt không trùng trong
    cùng giải**
...
-   **✅ Phân bảng tự động (Round-Robin Assignment):**
    ...
    -   **Phân đều teams vào các bảng**

    -   **Validate đủ số đội trước khi phân**
...
-   **✅ Theo dõi stats tự động:**
    ...
    -   **Points (Thắng +3, Hòa +1, Thua 0)**

    -   **Form (5 trận gần nhất: W/D/L)**

**4. Quản lý Lịch thi đấu & Kết quả (Match Management)**

-   **✅ Tạo lịch thi đấu tự động:**

    -   **Round-Robin Algorithm: Tối ưu, không trùng lặp**

    -   **Group-Stage: Lịch riêng cho từng bảng**

-   **✅ Xem lịch thi đấu (filter theo vòng, bảng, trạng thái)**

-   **✅ Cập nhật thông tin trận:**

    -   **Ngày giờ, sân đấu, trọng tài, ghi chú**

-   **✅ Cập nhật kết quả tỷ số:**

    -   **Tự động tính điểm cho 2 đội**

    -   **Tự động update stats (played, won, goals, points\...)**

    -   **Tự động update form (W/D/L)**

    -   **Có thể sửa kết quả cũ (revert stats rồi tính lại)**
...
-   **✅ Upload Highlight Videos:**
    ...
    -   **Upload file video từ máy lên Cloudinary**

    -   **Mỗi video tối đa 20MB (fix cứng)**

    -   **Số video tối đa = tổng bàn thắng (homeScore + awayScore)**

    -   **Có thể thêm title cho mỗi video**

    -   **Xóa từng video highlight**
...
-   **✅ Reset kết quả:**
    ...
    -   **Reset 1 trận: Xóa kết quả, stats về 0**

    -   **Reset toàn bộ giải: Xóa tất cả kết quả, stats về 0**
...
-   **✅ Bảng xếp hạng (Standings):**
    ...
    -   **Sắp xếp theo: Điểm \> Hiệu số \> Bàn thắng \> Tên**

    -   **BXH toàn giải (Round-Robin)**

    -   **BXH theo từng bảng (Group-Stage)**

    -   **BXH tất cả bảng**

    -   **Hiển thị: Position, Team info, Stats, Form**

-   **✅ Thống kê tổng quan giải đấu:**

    -   **Tổng số đội, trận, bàn thắng**

    -   **Số trận đã đấu / còn lại**

    -   **Trung bình bàn thắng/trận**

-   **✅ Top Rankings:**

    -   **Top Scorers: 5 đội ghi bàn nhiều nhất**

    -   **Best Defense: 5 đội để thủng lưới ít nhất**

    -   **Best Form: 5 đội phong độ tốt nhất (tính theo form 5 trận)**
...
-   **Password: Tối thiểu 8 ký tự, có chữ thường, chữ hoa, số, ký tự đặc
    biệt (@\$!%\*?&)**
...
**\"message\": \"Giải đấu này ở chế độ riêng tư. Bạn cần có mã truy
cập!\"**
...
POST /match/generate-schedule/{leagueId} Authorization: Bearer
{accessToken}
...
PUT /match/{matchId}/result Authorization: Bearer {accessToken}
Content-Type: application/json
...
\- ✅ Tự động update stats (played, won, drawn, lost, goals,
...
PUT /match/{matchId}/info Authorization: Bearer {accessToken}
Content-Type: application/json
...
PUT /match/{matchId}/status Authorization: Bearer {accessToken}
Content-Type: application/json
...
PUT /match/{matchId}/video Authorization: Bearer {accessToken}
Content-Type: application/json
...
POST /match/{matchId}/highlights Authorization: Bearer {accessToken}
Content-Type: multipart/form-data
...
\"message\": \"Số video highlight tối đa là 4 (bằng tổng số bàn thắng).
Hiện có 0 video.\"
...
DELETE /match/{matchId}/highlights/{highlightId} Authorization: Bearer
{accessToken}
...
PUT /match/{matchId}/photos Authorization: Bearer {accessToken}
Content-Type: multipart/form-data
...
\*\*Restriction:\*\* Không thể xóa trận đã có kết quả (status =
\"finished\")
...
\"message\": \"Không thể xóa trận đấu đã có kết quả! Vui lòng reset kết
quả trước.\"
...
DELETE /match/delete-schedule/{leagueId} Authorization: Bearer
{accessToken}
...
\"message\": \"Không thể xóa lịch vì đã có 5 trận có kết quả! Vui lòng
reset toàn bộ kết quả trước.\"
...
POST /user/register { \"username\": \"admin\", \"email\":
\"<admin@league.com>\", \"password\": \"Admin@123\" }

2\. \*\*Login:\*\*

POST /user/login { \"email\": \"<admin@league.com>\", \"password\":
\"Admin@123\" }
...
POST /league/create Authorization: Bearer eyJhbGc\... Content-Type:
multipart/form-data

name: Giải Bóng Đá Phủi 2024 type: round-robin visibility: public
numberOfTeams: 6 startDate: 2024-12-15 endDate: 2024-12-30
...
POST /match/generate-schedule/674d5678\... Authorization: Bearer
eyJhbGc\...
...
PUT /match/{matchId}/result Authorization: Bearer eyJhbGc\... {
\"homeScore\": 3, \"awayScore\": 1 }

\*\*Thêm video Full Match:\*\*

PUT /match/{matchId}/video Authorization: Bearer eyJhbGc\... {
\"videoUrl\": \"<https://youtube.com/watch?v=>\...\" }

\*\*Upload 4 highlight videos (3+1 bàn thắng):\*\*

POST /match/{matchId}/highlights Authorization: Bearer eyJhbGc\...
Content-Type: multipart/form-data

highlights: goal1.mp4 highlights: goal2.mp4 highlights: goal3.mp4
highlights: goal4.mp4 titles: Rashford mở tỷ số phút 15 titles: Bruno
nhân đôi cách biệt phút 32 titles: Rashford lập cú đúp phút 68 titles:
Sterling gỡ 1 bàn phút 85
...
\"drawn\": 1, \"lost\": 0, \"goalsFor\": 15, \"goalsAgainst\": 5,
\"goalDifference\": 10, \"points\": 13 }, \"form\": \[\"W\", \"W\",
\"D\", \"W\", \"W\"\] }, // \... 5 đội khác
...
\- Access Token hết hạn sau \*\*15 phút\*\* → Dùng Refresh Token để gia
hạn
...
**🔓 2. TRUY CẬP BẰNG ACCESS TOKEN (Dành cho người xem, không đăng
nhập)**
